...
We beginnen dit vak met een eenvoudige opdracht. We gaan in deze opdracht een histogram maken van een gegeven dataset. <br>
...
>Installeer de volgende twee bestanden naar een werkfolder op je computer:<br>
 ...
 en [DAS_DatasetGenerator.py](/opdrachten-module-1/DAS_DatasetGenerator.py).
 Open de bestanden in het VSCode programma en lees de code. We zullen hieronder wat uitleg erover geven. 

In `M1.A_MooiPlotten.py` wordt eerst een dataset aangemaakt met de volgende regel:
...
Nu kun je het bestand `M1.A_MooiPlotten.py` runnen. Kijk goed naar je output.
...
> Voor we iets gaan verander in  `M1.A_MooiPlotten.py`, voer eerst je naam en je studentnummer in in de eerste regels van dat bestand. Je moet de code uiteindelijk ook inleveren. We gebruiken deze bij het nakijken om, als er een fout is gemaakt in de opdracht te kunnen bekijken waar die vandaan komt. Het is natuurlijk ook de bedoeling dat de resultaten die je inlevert overeen komen met de resultaten uit je programmaatje. Daar kijken we ook naar.

In `M1.A_MooiPlotten.py` vind je de volgende regel code:
...
> Kopieer nu je **histogram** en je **code** in het document.  
